...
class SectionExecutor:
    ...
    def __init__(
        ...
        if llm_client is not None:
            ...
            # Auto-import the default LLM client (Groq + Ollama with auto-fallback)
    ...
    def run(self, section: SectionNode) -> dict:
        """
        Execute the write/critique/revise loop for one section.

        Returns a dict with:
          - approved (bool)       — True if threshold met or best-effort approved
          - final_score (float)   — best critic score achieved
          - iterations (int)      — number of write/critique cycles run
          - below_threshold (bool)— True if approved despite not meeting threshold
    ...
    def _call_writer(
        ...
        evidence_summary = self._format_evidence(brief.get("evidence_refs", []))
    ...
    def _approve_section(
        ...
        if below_threshold:
            self.repo.execute(
                ...
                SET quality_flag = 'below_threshold', updated_at = ?
                WHERE id = ?
                """,
